...
def init_db():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS cases (
        ...
        rag_mode TEXT,
        router_complete BOOLEAN, router_question TEXT,
...
def router_check_completeness(symptoms):
    if not GEMINI_AVAILABLE:
        return {"complete": True, "follow_up_question": None, "reason": "API unavailable"}
    prompt = f'''You are an intake nurse. A patient said: "{symptoms}"

Determine if this gives ENOUGH information for triage.
COMPLETE needs: SPECIFIC symptoms + some indication of duration or severity.
INCOMPLETE examples: "I don't feel well", "I'm sick", "I have pain", "help me"
COMPLETE examples: "Severe chest pain for 30 minutes radiating to left arm", "Cough for 3 weeks with yellow mucus"

Respond ONLY in this exact JSON format, nothing else:
{{"has_sufficient_info": true, "reason": "brief explanation", "follow_up_question": null}}
or
{{"has_sufficient_info": false, "reason": "brief explanation", "follow_up_question": "specific question to ask"}}'''
    try:
        raw = gemini_call(prompt, max_tokens=300)
        clean = re.sub(r'```json\s*|```\s*', '', raw).strip()
        result = json.loads(clean)
        return {"complete": result.get("has_sufficient_info", True), "follow_up_question": result.get("follow_up_question"), "reason": result.get("reason", "")}
    except:
        return {"complete": True, "follow_up_question": None, "reason": "Router error"}

def evaluator_enhance_triage(triage_reasoning, symptoms):
    if not GEMINI_AVAILABLE:
        return triage_reasoning
    prompt = f'''You are a senior emergency physician reviewing a junior colleague's triage assessment.

Patient symptoms: "{symptoms}"

Junior colleague's assessment:
{triage_reasoning}

Review and enhance by:
1. Check if the urgency level is appropriate
2. Add any critical red flags that were missed
3. Ensure next steps are clinically complete
4. Keep the same citation references [1], [2] etc - do NOT invent new sources

Output the ENHANCED assessment in the SAME format. Do NOT change the urgency level unless clearly wrong.'''
    try:
...
def parse_triage(text):
    ...
    for key, pat in [('reasoning',r'Reasoning:\s*\n?(.*?)(?=\n(?:Recommendation|Next|Patient|Sources):|\Z)'),('recommendation',r'Recommendation:\s*\n?(.*?)(?=\n(?:Next|Patient|Sources):|\Z)'),('next_steps',r'Next [Ss]teps:\s*\n?(.*?)(?=\n(?:Patient|Sources):|\Z)'),('patient_explanation',r'Patient [Ee]xplanation:\s*\n?(.*?)(?=\n(?:Sources):|\Z)')]:
        m = re.search(pat, text, re.DOTALL|re.IGNORECASE)
        if m: result[key] = m.group(1).strip()
    return result

def run_complete_pipeline(symptoms):
    router_result = router_check_completeness(symptoms)
    if not router_result['complete']:
        return {'status':'incomplete','router_complete':False,'router_question':router_result['follow_up_question'],'router_reason':router_result['reason']}
    triage = run_triage(symptoms)
    original_reasoning = triage.get('reasoning','')
    evaluator_enhanced = False
    if GEMINI_AVAILABLE:
        enhanced = evaluator_enhance_triage(original_reasoning, symptoms)
        if enhanced != original_reasoning:
            triage['reasoning'] = enhanced
            evaluator_enhanced = True
    return {'status':'complete','router_complete':True,'evaluator_enhanced':evaluator_enhanced,'original_reasoning':original_reasoning,'triage':triage}
...
def book_action(case_id, tier, symptoms=''):
    if tier == 'Urgent':
        ...
        if SCHEDULE_AVAILABLE:
            ...
            if slot is not None:
                book_slot(slot['Slot ID'])
                return {'status':'booked','type':'Scheduled Appointment','doctor':slot['Doctor'],'time':f"{slot['Date']} ({slot['Day']}) at {slot['Time']}",'dept':slot['Department'],'room':slot['Room'],'booking_id':slot['Slot ID'],'agent_decision':'routine_appointment','instructions':f"Appointment with {slot['Doctor']} on {slot['Date']} ({slot['Day']}) at {slot['Time']} in {slot['Room']}. Arrive 15 minutes early."}
        return {'status':'booked','type':'Scheduled Appointment','doctor':'Dr. Ahmed','dept':'General','time':(datetime.now()+timedelta(days=random.randint(2,10))).strftime('%B %d, %Y at %I:%M %p'),'room':f'G-{random.randint(101,104)}','booking_id':f'BK-{uuid.uuid4().hex[:6]}','agent_decision':'routine_fallback','instructions':'Arrive 15 minutes early.'}
    return {'status':'self_care_issued','type':'Self-Care Guidance','doctor':None,'time':None,'dept':None,'room':None,'booking_id':f'SC-{uuid.uuid4().hex[:6]}','agent_decision':'self_care','instructions':'Manage at home. Return if symptoms worsen.','guidance':'- Rest and hydrate\n- Monitor symptoms\n- OTC medication as needed\n- Return if worsening or fever > 38.5C'}
...
st.sidebar.markdown(f"**Agentic AI:** {'Enabled (Router + Evaluator)' if GEMINI_AVAILABLE else 'Disabled'}")
if SCHEDULE_AVAILABLE:
...
with st.sidebar.expander("Admin"):
    if st.button("Reset Database", type="secondary"):
        ...
        st.session_state.booked_slots = set(); st.rerun()
            ...
            if not pending:
                ...
                if case:
                    ...
                    with cl:
                        ...
                        with badge_cols[0]:
                            if case.get('router_complete'): st.success("Router: Info complete")
                            else: st.warning("Router: Incomplete")
                        with badge_cols[1]:
                            if case.get('evaluator_enhanced'): st.success("Evaluator: Enhanced")
                            else: st.info("Evaluator: Original")
                        with badge_cols[2]:
                        ...
                        if case.get('evaluator_enhanced') and case.get('llm_reasoning_original'):
                            with st.expander("View Evaluator Changes (Before/After)", expanded=False):
                                ...
                                with col_b:
                                    ...
                                    st.text_area("",value=(case.get('llm_reasoning_original',''))[:600],height=180,disabled=True,label_visibility="collapsed",key="before")
                                with col_a:
                                    st.markdown("**Enhanced by Evaluator:**")
                                    st.text_area("",value=(case.get('llm_reasoning',''))[:600],height=180,disabled=True,label_visibility="collapsed",key="after")
                                st.info("The Evaluator acts as a senior physician reviewing the RAG output - adding missed red flags and ensuring clinical safety.")
            ...
            if not reviewed: st.info("No reviewed cases yet.")
                ...
                for c in reviewed:
                    ...
                    with st.expander(f"{em} {c['case_id']} - AI: {c.get('llm_urgency','?')} -> Final: {c.get('final_tier','?')} ({lb})"):
                        ...
                        if c.get('booking_agent_decision'): st.markdown(f"**Agent Decision:** {c['booking_agent_decision']}")
